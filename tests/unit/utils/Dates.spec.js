  ...
  describe("Dates", () => {
    ...
    it("should init Dates class with default EN if language not found", () => {
      ...
      Object.defineProperty(global, "navigator", {
        value: { userLanguage: "toto" },
        writable: true
      });
      const dateWithDefaultLocale = new Dates(
        dummyDate.month(),
        dummyDate.year()
      );
    ...
    it("should return an array with days in a month", () => {
      const expectedDays = [...Array(31).keys()].map(day =>
        (day + 1).toString()
      );
      const daysFormatted = newDate
        .getDays()
    ...
    it("should return an array with years range", () => {
      expect(newDate.generateYearsRange(2018, 2)).toEqual([
        2016,
        2017,
        2018,
        2019,
  ...
  describe("Functions", () => {
    ...
    describe("initDate", () => {
      it.each([
        ...
        [null, { range: false, locale: { lang: en } }, undefined],
        [undefined, { range: false, locale: { lang: en } }, undefined],
        [
          new Date([2019, 5, 16]),
    ...
    describe("generateDate", () => {
      it.each([
        ...
        (selectedDate, locale, expectedResult) => {
          expect(generateDate(selectedDate, locale).format("MMM")).toEqual(
            expectedResult
          );
        }
      );
    ...
    describe("generateDateFormatted", () => {
      it.each([
        ...
        (selectedDate, locale, format, expectedResult) => {
          expect(generateDateFormatted(selectedDate, locale, format)).toEqual(
            expectedResult
          );
        }
      );
    ...
    describe("generateDateWithYearAndMonth", () => {
      it.each([[2018, 2, "2018-03"], [2019, 3, "2019-04"]])(
        "when year = %p, month = %p should return %p when formatted with YYYY-MM",
        (year, month, expectedResult) => {
          expect(
            generateDateWithYearAndMonth(year, month).format("YYYY-MM")
    ...
    describe("generateDateRange", () => {
      it.each([
        [
          "2019-5-10",
          "2019-5-14",
          [...Array(5).keys()].map(day => dayjs(`2019-5-1${day}`))
        ],
        [
          dayjs("2019-5-10"),
          dayjs("2019-5-14"),
    ...
    describe("generateDateRangeWithoutDisabled", () => {
      it.each([
        ...
        (dates, minDate, maxDate, expectedResult) => {
          const ranges = generateDateRangeWithoutDisabled(
            dates,
            minDate,
            maxDate
          );
    ...
    describe("generateMonthAndYear", () => {
      it.each([
        ...
        (value, currentDate, mode, expectedResult) => {
          expect(generateMonthAndYear(value, currentDate, mode)).toEqual(
            expectedResult
          );
        }
      );
    ...
    describe("transformDateForModel", () => {
      it.each([
        ...
        (date, format, range, expectedResult) => {
          expect(transformDateForModel(date, format, range)).toEqual(
            expectedResult
          );
        }
      );
    ...
    describe("getDefaultInputFormat", () => {
      ...
      ])("When type is %p, should return %p", (type, expectedResult) => {
    ...
    describe("getWeekDays", () => {
      ...
      ])("when lang equal %p, should return %p", (locale, expectedResult) => {
    ...
    describe("getRangeDatesFormatted", () => {
      it.each([
        ...
        [
          { start: "2019-5-15", end: undefined },
          { lang: en },
          "YYYY-MM-DD",
          "2019-05-15 ~ __"
        ],
        [
          { start: undefined, end: "2019-5-15" },
          { lang: en },
          "YYYY-MM-DD",
          "__ ~ 2019-05-15"
        ],
        ...
        (dates, locale, format, expectedResult) => {
          expect(getRangeDatesFormatted(dates, locale, format)).toEqual(
            expectedResult
          );
        }
      );
    ...
    describe("isDateAllowed", () => {
      [
        ...
        {
          description:
            "return true if date is validated by allowedDates and equal min date",
          data: {
            date: dayjs(new Date([2019, 5, 16])),
            allowedFn: date => date.getDate() === 16,
            minDate: "2019-5-16"
        ...
        {
          description:
            "return true if date is validated by allowedDates and equal max date",
          data: {
            date: dayjs(new Date([2019, 5, 16])),
            allowedFn: date => date.getDate() === 16,
            maxDate: "2019-5-16"
    ...
    describe("areSameDates", () => {
      it.each([
        ...
        (date, selectedDate, type, expectedResult) => {
          expect(areSameDates(date, selectedDate, type)).toEqual(
            expectedResult
          );
        }
      );
    ...
    describe("isBetweenDates", () => {
      it.each([
        ...
        (date, startDate, enDate, expectedResult) => {
          expect(isBetweenDates(date, startDate, enDate)).toEqual(
            expectedResult
          );
        }
      );
