...
describe("VDPickerYearMonth", () => {
  ...
  describe("watch", () => {
    beforeEach(() => {
      jest
        .spyOn(utilsFunction, "computeYearsScrollPosition")
        .mockReturnValue(100);
    });
    ...
    it("should scroll to active year if mode is year", async () => {
      ...
      const containerToScroll = wrapper.element.querySelector(
  ...
  describe("methods", () => {
    ...
    describe("isSelectedMonthOrQuarter", () => {
      it.each([
        ...
        [
          { mutableDate: dayjs(new Date([2019, 5, 16])), range: true },
          4,
          false
        ],
        [
        ...
        (props, monthIndex, expectedResult) => {
          const wrapper = mountComponent(props);
          expect(wrapper.vm.isSelectedMonthOrQuarter(monthIndex)).toEqual(
            expectedResult
          );
        }
      );
    ...
    describe("isMonthOrQuarterDisabled", () => {
      it.each([
        ...
        (allowedDates, mode, monthIndex, expectedResult) => {
          ...
          expect(wrapper.vm.isMonthOrQuarterDisabled(monthIndex)).toEqual(
            expectedResult
          );
        }
      );
